...
Please state here which option you are using: Option 1
...
g <- ggplot(dg, aes(x = home,
  ...
  labs(title = "Number of Times NBA Teams Played Each Other",
       x = "Home Team",
       y = "Away Team",
...
# add two cols to dg: division_away and division_home
  # then set the home and away teams to be factors with the levels in the order 
  # such that teams from the same division are clustered together.
...
dh <- df %>% 
  mutate(away = factor(away, levels = teams.order), home = factor(home, levels = teams.order))
...
g2 <- ggplot(dh, aes(x = home,
               y = away,
               fill = as.character(games))) +
  labs(title = "Number of Times NBA Teams Played Each Other",
       subtitle = "Ordered by Division",
       x = "Home Team",
       y = "Away Team",
       fill = "Number of Games") +
  geom_tile(color = 'black') +
...
```{r, figure.width = 4, fig.height = 10}
## your code here


g3 <-  ggplot(dh, aes(x = away,
               y = home,
               fill = as.character(games))) +
  labs(title = "Number of Times NBA Teams Played Each Other",
       subtitle = "Faceted by Division",
       x = "Home Team",
       y = "Away Team",
       fill = "Number of Games") +
  geom_tile(color = 'black') +
...
![](img/Screenshot 2024-02-06 181249.png)
